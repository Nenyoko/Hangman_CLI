...
int main(void){
    ...
    while (isplaying == 1){
        ...
        //if the player didn't guess one of the right letters, remove one of his life
        if(playerisright == false){
            playerlife--;
            std::cout << "Wrong letter, try again! Remaining life: " << playerlife << std::endl;
        }
        //if the player don't have any life, ask him if he wants to play one more game
        if(playerlife == 0){
        ...
        //if the word have been guessed by the player, ask him if he wants to play one more game
        if(wordisright == true){
            std::cout << wordtoguess_visiblebyplayer << std::endl;
